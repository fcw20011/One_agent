...
# Run daemon manually
uv run one-agent-core                        # foreground; Ctrl+C to stop
one-agent_PORT=8000 uv run one-agent-core        # override port

# Send a ping
uv run one-agent ping
uv run one-agent --version
```

## Architecture

This is a **dual-process** local AI agent system. `one-agent-core` is a persistent daemon; `one-agent` and `one-agent-tui` are clients that connect to it over a Unix domain socket.

```
one-agent-core (daemon)
  └─ listens on 127.0.0.1:7437 (TCP)
       ↑ JSON-RPC 2.0 NDJSON
one-agent (CLI)   one-agent-tui (TUI, S2+)
```

**`one-agent-tui` is the primary frontend.** All user-facing work on task management, observability, and interaction should be designed for and validated in the TUI first. The `one-agent` CLI exists only for quick scripted testing and debugging — it is not a product surface. When implementing features that touch the user interface, invest in the TUI layout, event rendering, and keyboard interactions. Do not shortcut TUI work by pointing to the CLI as an alternative.

### Protocol layer (`src/one_agent/core/bus/`)
...
### Transport layer (`src/one_agent/core/transport/`)
...
### Config (`src/one_agent/core/config.py`)

Four-tier priority: **built-in defaults → `~/.one-agent/config.toml` → `.env` → env vars**.
...
Relevant env vars: `one-agent_CONFIG`, `one-agent_HOST`, `one-agent_PORT`, `one-agent_LOG_LEVEL`, `one-agent_LOG_FILE`, `one-agent_LOG_FORMAT`.

### Daemon entry (`src/one_agent/core/app.py`)
...
Integration tests in `tests/conftest.py` spawn a real daemon subprocess using a random free port (via `free_port` fixture). The fixture finds a free port, releases it, passes it to the daemon via `one-agent_PORT`, then polls `asyncio.open_connection` until the daemon is ready.
